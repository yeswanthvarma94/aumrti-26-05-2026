...
import { Printer, FileSpreadsheet, FileText, Bot, CheckCircle2, XCircle, ChevronDown, ChevronRight, Loader2, Download, Mail } from "lucide-react";
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <Button size="sm" variant="outline" onClick={async () => {
                    ...
                    const { error } = await supabase.functions.invoke("email-tally-xml", {
                      body: { hospital_id: hospitalId, xml_content: xml, date_start: dateRange.start, date_end: dateRange.end },
                    });
                    if (error) { alert("Failed to send email: " + error.message); }
                    else { alert("Tally XML sent to billing email ✓"); }
                  }}>
                    <Mail className="h-4 w-4 mr-1" /> Email XML
                  </Button>
                </div>
              </div>
